...
template<typename TAspect, typename TParam,
        c_enable_if_t<std::is_base_of<DAspect, TAspect>::value, int>,
...
DElementPtr DElement::addDAspect(TParam* param) {
    ...
    DAspectPtr aspect = NAO_SAFE_MALLOC_NOBJECT(TAspect)
...
template<typename TAspect, typename ...Args,
        c_enable_if_t<std::is_base_of<DTemplateAspect<Args...>, TAspect>::value, int>>
DElementPtr DElement::addDAspect(Args... args) {
    if (!aspect_manager_) {
        aspect_manager_ = NAO_SAFE_MALLOC_NOBJECT(DAspectManager)
    }

    auto aspect = UAllocator::safeMallocTemplateNObject<DAspect>(std::forward<Args>(args)...);
    aspect->setBelong(this);
    aspect_manager_->add(aspect);
    return this;
}
...
template<typename T,
        c_enable_if_t<std::is_base_of<DElementParam, T>::value, int> >
