...
using NGRAPH_DEFAULT_FUNCTION = std::function<void()>;
using NGRAPH_DEFAULT_CONST_FUNCTION_REF = const std::function<void()>&;
using NGRAPH_NSTATUS_FUNCTION = std::function<NStatus()>;
...
/** 开启函数流程 */
#define NAO_FUNCTION_BEGIN                                           \
    NStatus status;                                                  \

/** 结束函数流程 */
#define NAO_FUNCTION_END                                             \
    return status;                                                   \

/** 无任何功能函数 */
#define NAO_EMPTY_FUNCTION                                           \
    return NStatus();                                                \
 ...
 * */
#define NErrStatus(info)                                             \
    NStatus(info, NAO_GET_LOCATE)                                    \

/** 返回异常信息和状态 */
#define NAO_RETURN_ERROR_STATUS(info)                                \
    return NErrStatus(info);                                         \

/** 根据条件判断是否返回错误状态 */
#define NAO_RETURN_ERROR_STATUS_BY_CONDITION(cond, info)             \
    if (unlikely(cond)) { NAO_RETURN_ERROR_STATUS(info); }           \

/** 不支持当前功能 */
#define NAO_NO_SUPPORT                                               \
    return NErrStatus(NAO_FUNCTION_NO_SUPPORT);                      \
...
/** 抛出异常 */
#define NAO_THROW_EXCEPTION(info)                                    \
    throw NException(info, NAO_GET_LOCATE);                          \
