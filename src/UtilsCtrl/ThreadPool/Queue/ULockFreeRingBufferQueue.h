...
template<typename T, NInt CAPACITY = NAO_DEFAULT_RINGBUFFER_SIZE>
...
public:
    ...
    ~ULockFreeRingBufferQueue() override { ring_buffer_.clear(); }
...
private:
    std::atomic<NInt>               head_;          // 开始元素（较早写入的）的位置
    std::atomic<NInt>               tail_;          // 尾部的位置
    std::vector<std::unique_ptr<T>> ring_buffer_;   // 环形队列
};

NAO_NAMESPACE_END

#endif   // NAO_ULOCKFREERINGBUFFERQUEUE_H
