/**
 * @FilePath     : /cola/src/UtilsCtrl/ThreadPool/Queue/UWorkStealingQueue.h
 * @Description  : 实现了一个包含盗取功能的安全队列
 * @Author       : naonao
 * @Version      : 0.0.1
 * @LastEditors  : naonao
 * @LastEditTime : 2024-06-20 19:52:30
 **/
#ifndef NAO_UWORKSTEALINGQUEUE_H
#define NAO_UWORKSTEALINGQUEUE_H
...
public:
     ...
     * @return
     */
    NVoid push(T&& value, NBool enable, NBool lockable)
    {
        if (enable && lockable) {
            mutex_.lock();
        }
     ...
     * @param values
     */
    NVoid push(const std::vector<T>& values)
    {
        while (true) {
            if (mutex_.try_lock()) {
     ...
     * @return
     */
    NBool tryPop(std::vector<T>& values, int maxLocalBatchSize)
    {
        bool result = false;
        if (!deque_.empty() && mutex_.try_lock()) {
     ...
     * @return
     */
    NBool trySteal(std::vector<T>& values, int maxStealBatchSize)
    {
        bool result = false;
        if (!deque_.empty() && mutex_.try_lock()) {
    ...
    NAO_NO_ALLOWED_COPY(UWorkStealingQueue)
...
#endif   // NAO_UWORKSTEALINGQUEUE_H
