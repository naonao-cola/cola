...
{
    NAO_FUNCTION_BEGIN
    status = (SIZE >= 0) ? thread_pool_->createSecondaryThread(SIZE) : thread_pool_->releaseSecondaryThread((-1) * SIZE);
...
template<NInt SIZE>
NStatus DCoordinator<SIZE>::checkSuitable(){NAO_FUNCTION_BEGIN NAO_RETURN_ERROR_STATUS_BY_CONDITION(loop_ > 1, "cannot set GCoordinator loop > 1")
