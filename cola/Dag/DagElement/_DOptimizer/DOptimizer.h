...
protected:
    ...
    static std::vector<std::vector<int>> buildGraph(const DSortedDElementPtrSet& elements,
        ...
        for (const auto& path : paths) {
            for (NSize i = 0; i < path.size() - 1; i++) {
                // 这里的 find是一定能找到的。因为path的数据，是从elements中记录的
                NSize height = std::distance(elements.begin(), elements.find(path[i]));
                for (NSize j = i + 1; j < path.size(); j++) {
                    NSize column = std::distance(elements.begin(), elements.find(path[j]));
