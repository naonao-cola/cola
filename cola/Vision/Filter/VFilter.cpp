...
cv::Mat __ideal_low_kernel(cv::Mat& scr, float sigma)
{
    // CV_32FC1
    ...
    for (int i = 0; i < scr.rows; i++) {
        for (int j = 0; j < scr.cols; j++) {
            // 分子,计算pow必须为float型
            float d = sqrt(pow(static_cast<float>(i - scr.rows / 2), 2) + pow(static_cast<float>(j - scr.cols / 2), 2));
            if (d <= d0) {
...
cv::Mat VFilter::ideal_low_pass_filter(cv::Mat& src, float sigma)
{
...
cv::Mat __ideal_high_kernel(cv::Mat& scr, float sigma)
{
    // CV_32FC1
    ...
    for (int i = 0; i < scr.rows; i++) {
        for (int j = 0; j < scr.cols; j++) {
            // 分子,计算pow必须为float型
            float d = sqrt(pow(float(i - scr.rows / 2), 2) + pow(static_cast<float>(j - scr.cols / 2), 2));
            if (d <= d0) {
...
cv::Mat VFilter::ideal_high_pass_filter(cv::Mat& src, float sigma)
{
    cv::Mat padded       = __image_make_border(src);
...
cv::Mat __butterworth_low_kernel(cv::Mat& scr, float sigma, int n)
{
    // CV_32FC1
    ...
    for (int i = 0; i < scr.rows; i++) {
        for (int j = 0; j < scr.cols; j++) {
            // 分子,计算pow必须为float型
            double d                             = sqrt(pow(static_cast<float>(i - scr.rows / 2), 2) + pow(static_cast<float>(j - scr.cols / 2), 2));
...
cv::Mat VFilter::butterworth_low_paass_filter(cv::Mat& src, float d0, int n)
{
    /*
...
cv::Mat __butterworth_high_kernel(cv::Mat& scr, float sigma, int n)
{
    // CV_32FC1
    cv::Mat butterworth_low_pass(scr.size(), CV_32FC1);
    // 半径D0越小，模糊越大；半径D0越大，模糊越小
...
cv::Mat VFilter::butterworth_high_paass_filter(cv::Mat& src, float d0, int n)
{
...
cv::Mat __gaussian_low_pass_kernel(cv::Mat scr, float sigma)
{
    // CV_32FC1
    ...
    for (int i = 0; i < scr.rows; i++) {
        for (int j = 0; j < scr.cols; j++) {
            // 分子,计算pow必须为float型
            float d = pow(float(i - scr.rows / 2), 2) + pow(static_cast<float>(j - scr.cols / 2), 2);
...
// 高斯低通滤波器
cv::Mat VFilter::gaussian_low_pass_filter(cv::Mat& src, float d0)
{
    cv::Mat padded          = __image_make_border(src);
...
cv::Mat __gaussian_high_pass_kernel(cv::Mat scr, float sigma)
{
    // CV_32FC1
    cv::Mat gaussianBlur(scr.size(), CV_32FC1);
    float   d0 = 2 * sigma * sigma;
    for (int i = 0; i < scr.rows; i++) {
        for (int j = 0; j < scr.cols; j++) {
            // 分子,计算pow必须为float型
            float d                      = pow(static_cast<float>(i - scr.rows / 2), 2) + pow(static_cast<float>(j - scr.cols / 2), 2);
...
// 高斯高通滤波器
cv::Mat VFilter::gaussian_high_pass_filter(cv::Mat& src, float d0)
{
    cv::Mat padded          = __image_make_border(src);
...
cv::Mat VFilter::cascade_filter(const cv::Mat& src, int K, int ksize)
{
    int     img_size = src.cols * src.rows;
    cv::Mat median_dst;
    cv::Mat diff_dst;
...
double VFilter::compute_PSNR(const cv::Mat& Mat1, const cv::Mat& Mat2)
{
    ...
    double      mse           = (sse / static_cast<double>(total_element));
    double      psnr          = 10.0 * log10(static_cast<double>(255) * 255 / (mse + 0.0000001));
    return psnr;
}
...
double VFilter::variance_laplacian(const cv::Mat& src)
{
    cv::Mat laplacain;
    ...
    double ret = (static_cast<double>(*stddev.ptr<float>(0, 0))) * (static_cast<double>(*stddev.ptr<float>(0, 0)));
    return ret;
}
