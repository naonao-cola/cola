 ...
 * @Version      : 0.0.1
 * @LastEditors  : naonao
 * @LastEditTime : 2024-08-19 14:57:03
...
public:
    ...
    static std::vector<cv::Point2f> gauss_spot_center(const cv::Mat& src)
    {
        ...
        for (NInt i = 0; i < contours.size(); i++) {
...
private:
    ...
    static bool initData(cv::Mat img, Eigen::VectorXf& vector_A, Eigen::MatrixXf& matrix_B)
    {
        ...
        while (i < img.cols) {
            j = 0;
            while (j < img.rows) {
                NFloat value = *img.ptr<NFloat>(i, j);
                if (value <= 0.5) {
                    value = 1;
                }
     ...
     * @return
     */
    static bool getCentrePoint(NFloat& x0, NFloat& y0, const Eigen::VectorXf vector_A, const Eigen::MatrixXf matrix_B)
    {
        // QR分解
